...
Slitly modified version of Update-HostIP. 
Adds an IPv6 adress to a existing host that has only IPv4 adresses. 
...
if ( $DirectChange ) {
    ...
    $Hosts | ForEach-Object {
        ...
        #Json body. "ipv6addr" == overwrite. "ipv6addr+" == add. "ipv6addr-" == delete. 
        $JsonBody = '{"ipv6addrs": [
            {
              "ipv6addr": "' + $NieuweIPAddr + '"
            }
          ]}
